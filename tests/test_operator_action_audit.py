...
Phase 0.8a per ADR-002. Verifies OPERATOR_ACTED emission for the four
existing Beta 10 operator-action endpoints:

  port_switch / whatif_apply / whatif_clear / send_brief
...
class TestProductionSafetyPath:
    ...
    def test_imports_with_psycopg_blocked(self):
        ...
        try:
            ...
            assert oaa.ACTION_PORT_SWITCH == "port_switch"
    ...
    def test_emit_async_is_noop_when_database_url_unset(self, monkeypatch):
        ...
        oaa.emit_async(
            "ams-demo", oaa.ACTION_PORT_SWITCH,
            summary="Switched active port to MELBOURNE",
            surface="api_set_port",
            actor_handle="O-1", port_id="MELBOURNE",
        )
        time.sleep(0.05)
...
class TestPayloadShape:
    ...
    def test_required_fields_present(self):
        import operator_action_audit as oaa
        payload = oaa.make_payload(
            oaa.ACTION_PORT_SWITCH,
            summary="Switched active port to BRISBANE",
            surface="api_set_port",
        )
    ...
    def test_payload_omits_optional_fields_when_none(self):
        ...
        payload = oaa.make_payload(
            oaa.ACTION_PORT_SWITCH,
            summary="x", surface="api_set_port",
        )
...
class TestSubjectMapping:
    ...
    def test_port_switch_targets_system_port(self):
        import operator_action_audit as oaa
        s = oaa._subject_for_action(
            oaa.ACTION_PORT_SWITCH, conflict_id=None, port_id="MELBOURNE")
        assert s == ("system", "MELBOURNE")
...
class TestNoMutation:
    ...
    def test_make_payload_returns_fresh_dict(self):
        import operator_action_audit as oaa
        p1 = oaa.make_payload(
            oaa.ACTION_PORT_SWITCH, summary="x", surface="api_set_port")
        p2 = oaa.make_payload(
            oaa.ACTION_PORT_SWITCH, summary="x", surface="api_set_port")
...
class TestOperatorActedEmission:
    ...
    def test_port_switch_creates_row(self, oaa_enabled, conn, tenant_id):
        oaa = oaa_enabled
        ok = oaa.emit_sync(
            tenant_id, oaa.ACTION_PORT_SWITCH,
            summary="Switched active port to MELBOURNE",
            surface="api_set_port",
            actor_handle="O-1", port_id="MELBOURNE",
        )
        ...
        assert subj_id == "MELBOURNE"
        assert actor_t == "operator"
        assert actor_h == "O-1"
        assert a_type == "port_switch"
        assert surface == "api_set_port"
        assert pid == "MELBOURNE"
...
class TestLifecycleSafety:
    ...
    def test_payload_isolated_from_caller_mutation(
        ...
        # primitives that are then frozen via make_payload. We
        # verify the snapshot guarantee by mutating the *summary string*
        # variable (impossible since strings are immutable in Python)
        # and a *port_id string*. The real test is that subsequent
        # calls do not bleed metadata into earlier audit rows.
        oaa.emit_async(
            tenant_id, oaa.ACTION_PORT_SWITCH,
            summary="Switched active port to MELBOURNE",
            surface="api_set_port",
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT payload->>'port_id' "
                "FROM audit.events WHERE tenant_id = %s ORDER BY sequence_no",
                (tenant_id,),
            )
            ports = [r[0] for r in cur.fetchall()]
        assert ports == ["MELBOURNE", "BRISBANE"], (
            f"per-emit payload isolation broken: {ports}"
        )
